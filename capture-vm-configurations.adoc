...
---
= Capturez vos configurations VM actuelles
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
. Dans la mosaïque VMware, sélectionnez *planifier et déployer*.
+
image:screenshot-choose-method.png["Une capture d'écran des différentes méthodes permettant de collecter les informations de configuration actuelles de votre machine virtuelle."]
...
.. Exécutez le script du collecteur de données que vous avez téléchargé et spécifiez l'option de collecte de données pour les statistiques des machines virtuelles « quotidiennes » ou « horaires ».
+
...
+
Pour plus de détails, reportez-vous au chapitre “Paramètres de ligne de commande” dans le https://resources.robware.net/resources/prod/RVTools.pdf["Documentation RVTools"^] .
...
====
.Et la suite ?
link:launch-onboarding-advisor.html["Créez un plan de déploiement à l'aide du conseiller de migration"].
