...
+
Le centre de planification est affiché. Depuis le centre de planification, vous pouvez importer vos données d'inventaire de machines virtuelles, consulter les importations de données d'inventaire précédentes et planifier une migration à l'aide de n'importe quel ensemble de données d'inventaire de machines virtuelles, puis sélectionner *Migration vers le calcul natif AWS*. L'écran d'accueil de l'assistant de migration VMware s'affiche.
